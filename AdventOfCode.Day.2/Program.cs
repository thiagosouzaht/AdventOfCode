...
var sumOfValidReport = reports.Where(report => IsReportSafe(report)).Count();

var listOfInvalidReports = reports.Where(report => !IsReportSafe(report));

var sumOfInvalidButValid = 0;

foreach (var report in listOfInvalidReports)
{
    var isValid = false;
    ...
    for (int i = 0; i < report.Count(); i++)
    {
        ...
        if (!isValid && IsReportSafe(reportWithoutCurrentValue)) isValid = true;
    }
